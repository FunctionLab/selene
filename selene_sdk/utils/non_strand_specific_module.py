...
class NonStrandSpecificInteract(Module):
    """
    A torch.nn.Module that wraps a user-specified model architecture if the
    architecture does not need to account for sequence strand-specificity.
    ...
    def __init__(self, model, mode="mean"):
        super(NonStrandSpecificInteract, self).__init__()
    ...
    def forward(self, input):
        reverse_input = 5 - input
        reverse_input[reverse_input == 5] = 0

        output = self.model.forward(input)[0]
        output_from_rev = self.model.forward(reverse_input)[0]
        if type(output) == tuple:
            output1, output2 = output
            output_from_rev1, output_from_rev2 = output_from_rev
            return ((output1 + output_from_rev1) / 2, (output2 + output_from_rev2) / 2)
