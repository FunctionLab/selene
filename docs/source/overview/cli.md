...
- `metrics`: Default is `dict(roc_auc=sklearn.metrics.roc_auc_score, average_precision=sklearn.metrics.average_precision_score)`. A dictionary that maps metric names (`str`) to metric functions. In addition to the [loss function you specified with your model architecture](#expected-input-class-and-methods), these are the metrics that you would like to monitor during the training/evaluation process (they all get reported every `report_stats_every_n_steps`). For another example of how
  this parameter is used, please review our [Regression Models in Selene](https://github.com/FunctionLab/selene/blob/master/tutorials/regression_mpra_example/regression_mpra_example.ipynb) tutorial. You can `!import` metrics from `scipy`, `scikit-learn`, `statsmodels`.  
