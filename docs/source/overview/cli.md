...
- [Sampler configurations](#Sampler-configurations)
    - Samplers for training (and evaluation, optional):
...
There are also various configuration files associated with the Jupyter notebook [`tutorials`](https://github.com/FunctionLab/selene/tree/master/tutorials) and [`manuscript`](https://github.com/FunctionLab/selene/tree/master/manuscript) case studies that you could use as a starting point.
...
- `class_args`: The arguments needed to instantiate the class.
...
For training, evaluation, and analysis [of sequences using trained models], Selene requires that specific keys in the YAML file correspond to function-type values. The value is used to construct an object that is a class in `selene_sdk`. Our [documentation website](http://selene.flatironinstitute.org/) is an important resource for debugging configuration-related errors when you run Selene via the CLI. 

We have tried to cover as many of the common configurations as we can in this document. 
...
- `n_test_samples`: Default is `None`. Specify the number of test samples in the test set. If `None`
    ...
    - and the sampler you use is of type `selene_sdk.samplers.MultiFileSampler`, we will use all the test samples available in the appropriate data file. 
...
- `output_dir` can be specified as a top-level key in the configuration. You can specify it within each function-type constructor (e.g.  `!obj:selene_sdk.TrainModel`) if you prefer. If `output_dir` exists as a top-level key, Selene does use the top-level `output_dir` and ignores all other `output_dir` keys.
...
#### Additional notes
Similar to the `train_model` configuration, any arguments that you find in [the documentation](http://selene.flatironinstitute.org/selene.html#evaluatemodel) that are not present in the function-type value's arguments are automatically instantiated and passed in by Selene.
...
- `test_performance.txt`: columns are [`class`, `roc_auc`, `average_precision`] (eventually, we will support more scores and allow you to specify which scores you want). The breakdown of performance metrics by each class that the model predicts.
...
- `precision_recall_curves.svg`: An AUPRC figure that we generate for you. Each curve corresponds to one of the classes the model predicts.
- `roc_curves.svg`: An ROC AUC figure that we generate for you. Each curve corresponds to one of the classes the model predicts.
...
### Samplers used for training (and evaluation, optional)
...
You can use all the samplers specified for training for evaluation as well. Additionally, you can use single-file samplers, which we describe below. 
