...
if __name__ == "__main__":
    ...
    output_dir = "/home/kc31/fi-projects/deepsea-pipeline/data/labmtg/on_mouse"

    mouse_samples = "/home/kc31/fi-projects/deepsea-pipeline/data/labmtg_mm10/input_sequences_full.txt"

    hg19_dir = "/tigress/kc31/compress_hg19_latest"
    hg19_features = os.path.join(hg19_dir, "distinct_features.txt")
    hg19_features_list = get_features_list(hg19_features)

    mouse_samples_list = None
    with open(mouse_samples, 'r') as fh:
        lines = fh.readlines()
        mouse_samples_list = [l.strip() for l in lines]

    sequence_encodings = [
        _sequence_to_encoding(s, {'A': 0, 'C': 1, 'G': 2, 'T': 3})
        for s in mouse_samples_list
    ]
    sequence_encodings = np.array(sequence_encodings)

    resnet_model = DeepSEA_Res(1001, len(hg19_features_list))
    ...
    batch_size = 16
    evaluate(resnet_model, batch_size, sequence_encodings, hg19_features_list,
